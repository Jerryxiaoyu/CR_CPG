...
from CPG_core.controllers.CPG_controller_snake_sin import CPG_network, CPG_network5
...
def oscillator_nw(position_vector, max_time=10.0, fitness_option=6, plot = False, log_dis = False, render = False, monitor_path=None, save_plot_path = None, env_name= None):
    ...
    if env_name is  None:
        env = gym.make('CellrobotSnakeEnv-v0')
    else:
    ...
    CPG_controller  = CPG_network5(CPG_node_num, position_vector)
   ...
   # robot_handle.set_angles_slow(target_angles=initial_bias_angles, duration=2.0, step=0.01)
    ...
    for t in np.arange(0.0, max_time, dt):
        ...
        if t >= 10 and t <= 12:
            for i in range(1, CPG_controller.num_CPG):
                CPG_controller.CPG_list[i].parm['f12'] += 0 #pi / 2.0 / 2.0 * dt
...
# position_vector = np.zeros(28)
# position_vector[0] = 1
# for i in range(1, 10):
#     position_vector[i] = 1  # gain
# for i in range(10, 19):
#     position_vector[i] = 0  # bias
# for i in range(19, 28):
#     position_vector[i] =  0  # phase
#
# print(position_vector)
#
# oscillator_nw(position_vector, plot=True, render=True, monitor_path=None,
