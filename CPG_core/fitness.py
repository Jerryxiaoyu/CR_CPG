...
def calc_fitness(start_x, start_y, start_z,
    ...
    if log_dis:
        log.info('[FIT] Executing calc_fitness with fitness_option: {0}'.format(fitness_option))
        log.info('[FIT] Other arguments:')
        log.info('[FIT] start_x={0}, start_y={1}, start_z={2}'.format(start_x, start_y, start_z))
        log.info('[FIT] end_x={0}, end_y={1}, end_z={2}'.format(end_x, end_y, end_z))
        log.info('[FIT] avg_z={0}'.format(avg_z))
        log.info('[FIT] up_time={0}'.format(up_time))
    ...
    if fitness_option==1:
        ...
        fitness = x_distance
        if log_dis:
            log.info('[FIT] fitness = x_distance')
        ...
        if log_dis:
            log.info('[FIT] fitness = x_distance + up_time/60.0')
        ...
        if log_dis:
            log.info('[FIT] fitness = x_distance*0.3 + up_time*0.7')
        ...
        if log_dis:
            log.info('[FIT] fitness = x_distance + up_time*0.5')
